...
| writer (-> `page.json` x N) | `harness/write.py`, `harness/repair.py` | `write_page` (in `write.py`) is cartridge-agnostic: loads `cartridge.md`, `schema.json`, up to 2 trimmed exemplars. `write_and_gate_page` and `check_page_gates` -- the repair loop itself -- live in `repair.py` (R2 split, 2026-09-11). |
| deterministic pre-repair | `harness/repair.py` | `apply_deterministic_fixes` -- hype-word substitution, incidental-numeral conversion, warranty-sentence fix -- resolves specific gate failures without spending a model call. |
| renderer (-> `index.html`) | `harness/render.py` | Injects byline/dates/disclosure/ad-label and JSON-LD; the model never writes these fields. Fix cycle 23: `harness/structure.css` (renamed from `fallback.css`) always loads first as the structural/component/responsive layer; a tenant's own `brand/base.css`, if present, loads second as an override (tokens/fonts/colors), never a replacement. Falls back to a built-in byline if `brand/byline.html` is missing. |
...
| CLI orchestration | `harness/cli.py` | `build_parser` and the `cmd_*` command bodies only -- the repair loop moved to `repair.py`, REVIEW.md rendering to `review_md.py` (R2 split, 2026-09-11). |
