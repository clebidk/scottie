...
## Cycle 42 — paragraph-to-image matching + vision-drafted alt text (2026-09-18)
- Feature 1: `ground.match_images_to_text(page, assets, *, cartridge_name, exclude_ids, allow_ai_renders)` -- a deterministic (no model call) render-time pass, run right after `enforce_slot_plan` and before `record_used_asset_ids` in `render.render_page` (same `download_assets=True` gate). Where `enforce_slot_plan` only fixes selection-policy violations, this asks a content question: does the image next to a paragraph actually show what the paragraph talks about? Scores each eligible candidate's REVIEWED alt (`tenants/<t>/brand/asset-review.json`) + tags parsed from its review `note` (weight 2 each) plus kind/title (weight 1 each) against the slot's own heading+body text (article: paired with `body_sections[i]` by index; longform: `hero.headline`+`subhead` and each step's `title`+`text`; product-page: `hero.product_name`+`promise`; listicle: each reason's `heading`+`text`) -- token overlap, lower-cased, stopwords dropped, trailing plural "s" stripped. Replaces the writer's own pick only when a candidate scores >= 2 AND strictly beats the current pick (a tie keeps the writer's choice); never introduces a duplicate on the page or across `exclude_ids`; never selects a never-eligible kind or a disallowed `ai_render`. A no-op -- `page.json` byte-identical to before this cycle -- for any tenant with no reviewed alt text anywhere. Decisions land in the existing `<run_dir>/.image-selection.json` under a new `"matches"` key (`ground.record_image_matches`, keyed by cartridge like `record_used_asset_ids`'s own entries), and one `log.event` line per replacement.
- Feature 2: `harness images describe --tenant <t> --model <slug> [--limit N] [--force] [--dry-run]` (new `harness/asset_describe.py`) vision-drafts alt text + tags for a model's not-yet-reviewed asset pool (`ground.full_asset_pool`, minus already-excluded/already-alt'd assets unless `--force`) and writes them into `asset-review.json` in the same shape a human reviewer's save writes (`by: "vision-draft"`). Reuses `render.download_asset` into the same `runs/asset-cache/` the `/images` review site uses, then resizes to <=1200px and re-encodes as JPEG in memory with Pillow before the vision call. Model: `tenant.yaml`'s `models.vision`, else `claude-haiku-4-5-20251001` -- `harness/pricing.py` only prices bare model-family ids, so a dated override is matched by prefix for cost-ledger bookkeeping only (`asset_describe._pricing_model_id`); the API call itself always uses the exact configured id. Prompt is tenant-neutral (built from the product's own name/title -- no "category" field exists in this schema; forbidden terms pulled from the tenant's own `vocab.yaml` `emf_terms`, never hardcoded), always instructs "describe only what is visible; no health claims; never mention EMF", and `strip_forbidden_terms` is a backstop over the model's own drafted name/alt. Response tags are filtered to a fixed `ALLOWED_TAGS` list. Saved atomically (`asset_review.save_asset_review`) every 10 described assets and once more at the end, so a crash mid-batch keeps most of its progress. Costed like `harness ingest`/`harness run`: `budget.reserve_spend` gates the whole invocation against the tenant's daily cap BEFORE a client is even constructed (exit 3, nothing spent, if already over cap); `budget.record_spend` records the actual cost once at the end. `harness images pool --tenant <t>` prints per-model total/reviewed/excluded/with-alt counts (the same totals the `/images` review site's index shows, plus a with-alt column).
- Known gap, flagged rather than silently worked around: the scoring rule's "+1 when the asset's model equals the run's product slug" bonus can't fire against today's data -- `ground.select_drive_assets`/`select_listicle_pack_assets` (which build `facts_for()`'s capped `assets` list, the one `match_images_to_text` actually scores against) never attach a `model` field to their returned dicts, only `ground.full_asset_pool`'s uncapped rows do. Adding one to the capped list was considered and rejected: `test_facts_pack_stays_small` caps `facts_pack` at ~4k tokens and currently sits at ~3989 -- eleven tokens of headroom, not enough for a `"model": "<slug>"` key on up to 12 assets. The bonus is implemented defensively (`asset.get("model")`) so it activates on its own if that ever changes, and is covered by its own unit test against a synthetic asset that does carry the field.
...
## Cycle 66 (env guard for module- and session-scoped fixtures, 2026-09-23)
