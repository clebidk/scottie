...
call inside `harness run`):

| Drive folder id | Local index file | Covers |
|---|---|---|
| `16gomD6ar2ZasJTepvuZv-aLT2GorLvz1` | `tenants/peak-saunas/brand/assets.json` | The full brand library, every model (lifestyle/render/installation/video/logo/ugc), 1,046 files indexed. |
| `1TJIKdHrD4AheKYq-0D4dLnbGfZrFwIVA` | `tenants/peak-saunas/brand/assets-listicle-pack.json` | A separate, curated pack for the listicle cartridge -- Mini and Matterhorn only, 105 files (`photo_product`, `photo_install`, `still_video`, `ai_render`). |
...
`harness/ground.py`'s `LocalFactsSource.facts_for()` assembles one `assets` list per run, in
this order:

1. **Shopify product images** (`select_drive_assets`'s sibling, built from
   `tenants/peak-saunas/claims/products.json`'s own `image_urls`) -- always first, so the writer's hero slot
   defaults to a real product photo already on the live site.
2. **`tenants/peak-saunas/brand/assets.json` Drive assets** (`select_drive_assets`) -- up to 6, tiered
   lifestyle/interior first, then render, then installation; `video`/`logo`/`ugc` and any
   row flagged `excluded` are never eligible.
3. **`tenants/peak-saunas/brand/assets-listicle-pack.json` assets** (`select_listicle_pack_assets`, fix cycle
   ...
   preferred; an `ai_render` is only ever eligible when `tenants/peak-saunas/claims/config.json`'s
   ...
   `harness/render.py`'s `asset_alt()` -- the renderer's own field, so the model can't drop
   or reword it.
...
`harness/render.py`'s `render_page()` downloads only the assets the page actually references
(`collect_asset_ids`) into `tenants/peak-saunas/out/<run-id>/<cartridge>/assets/<asset-id>.<ext>`, rewrites
...
`harness review tenants/peak-saunas/out/<run-id>` then inlines each cartridge's images as `data:` URIs into
