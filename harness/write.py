...
Product names: name a product by facts_pack.product.full_name (another model by its own full_name) on its FIRST mention in each major section (hero, each FAQ answer, each step, etc.) -- write it exactly as given, same letters and same capitals -- and by facts_pack.product.name (the model name alone) or "the sauna"/"this model" after that; every CTA text uses the model name alone. Never write facts_pack.product.seo_title, or any model name followed by its capacity/style words (the model name then "2-Person ...", "1-Person Infrared Sauna" and the like), as a product's name -- a capacity is a spec, so write it as its own phrase ("a two-person cabin") with the capacity spec row's claim_id in that sentence's "claim_ids" array (the JSON field; the id itself is never printed as text). Never put the company name {company!r}, or any former company name, in front of a model name in any other form than full_name gives it. WRONG: "the full seo_title (spec-model-capacity), which is priced at $8,250" -- the long title is not a name and the id in parentheses is a bug, not a citation. RIGHT: "the <full_name>, which is priced at $8,250" with "claim_ids": ["price-model"] on that sentence's own JSON node.
...
def _append_warmup_hard_constraints(hard_constraints, cartridge_name, tenant):
    ...
    hard_constraints.append(
        ...
        f"{company!r}, the tenant short name, facts_pack.product.full_name, the product name, "
        ...
        "close (full_name there). Ignore the global voice full_name-on-first-"
