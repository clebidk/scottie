...
## Structure (in order)
1. Header block: the tenant's optional disclosure label (`disclosure_label`; none when unset), eyebrow, headline, subhead, optional proof-stat row, one CTA, optional risk-reversal microcopy line, byline block (`{{ byline_html | safe }}`).
