...
2. Hero: product short_name, one-line promise taken from the ad angle, price, financing line ("Financing available" unless a lender is configured). Hero image from asset library. CTA is `cta_text`/`cta_url` (top-level, see Rules) -- e.g. "Buy / Shop <model>".
...
## Rules
- 250–500 words. Return top-level `cta_text` and `cta_url` exactly once -- the renderer reuses them verbatim in the hero and the repeat CTA. Never invent a second CTA text anywhere on the page. No reviews carousel, no related products, no nav, no newsletter.
