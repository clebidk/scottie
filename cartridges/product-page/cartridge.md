...
2. Hero: product short_name, one-line promise taken from the ad angle, price, financing line ("Financing available" unless a lender is configured). Hero image from asset library. CTA is `cta_text`/`cta_url` (top-level, see Rules) -- exactly one of schema.json's `allowed_cta_texts`, e.g. "Shop the Peak Fuji 2-Person Infrared Sauna" or "Buy the Peak Fuji 2-Person Infrared Sauna".
