...
2. Hero: headline from the ad angle, subhead, hero image, financing line if the ad used price. CTA is `cta_text`/`cta_url` (top-level, see Rules) -- do not add a separate cta object here.
...
## Rules
- 800–1,400 words. Return top-level `cta_text` and `cta_url` exactly once -- the renderer reuses them verbatim in the hero, sticky bar, and final block. Never invent a second CTA text anywhere on the page.
