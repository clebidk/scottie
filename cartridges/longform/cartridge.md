...
- Proof row (`hero.proof_stats`, optional): 2–3 stats, each `{value, label, claim_ids}`, directly under the hero subhead, before the first body section -- verified claims only (e.g. live review rating/count, warranty term, free shipping). Every stat needs at least 1 claim_id; omit the row entirely rather than invent or pad a stat with nothing to cite.
