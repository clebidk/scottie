...
2. Hero: headline from the ad angle, subhead, hero image, financing line if the ad used price. CTA is `cta_text`/`cta_url` (top-level, see Rules) -- exactly one of schema.json's `allowed_cta_texts`, e.g. "See pricing", or a "Shop the ..." option; do not add a separate cta object here.
