...
Build it by ADAPTING a proven listicle, not by inventing long advertorial copy: the tenant's listicle exemplar (its shipped winner) is the model. Keep the winner's component map (hero / proof / item rhythm / mid-page CTA / close), its section density and its scannable voice; swap brand, images (asset_ids), tone and claims to this tenant and this ad. The Structure below stays fixed -- the winner sets the rhythm and density inside it. Do not pad toward article length.
...
Headline templates (cycle 70): a run may instead name one of the headline templates in `cartridges/listicle/headlines.yaml` (docs/HEADLINES.md). The hard constraints then quote it, with the rules for each slot and the claim_ids its items must cite -- follow that template instead of the formula in the table. Only its `<brand>` slot may name the company, and a `<product>` slot may name a model by its full name.
...
- No claims outside verified_claims. Health statements cite the study and its population; never promise an outcome for {{ tenant.name }} hardware. A winner's unsourced trust line or number is a pattern to remake with verified facts only -- never paste it unless the same fact is in this page's facts_pack.verified_claims.
...
- Images: `hero.asset_id` plus one per item, from the asset library, lifestyle or installation over stock/illustration, referenced by asset_id only -- the renderer derives alt text, never write your own "alt" field. Every slot on the page is a distinct asset. No before/after, no clinical settings. When you can, give each slot an asset that plays the same role as the winner's slot there (hero / item / lifestyle), not a random pick.
...
- Voice: plain, specific, second person or first person. Short sentences, one idea per item -- match the winner's punchiness, never expand into advertorial depth. No exclamation marks. No "game-changer", "unlock", "elevate", "journey". A headline template that carries one of these words itself (h16's "Game-Changer") may use it in the headline only.

## From the ad
Hook and angle -> headline and dek. The ad's own list of features/objections/benefits -> the item set, one per item, in the order that reads best (not necessarily the ad's own order), mapped onto the winner's item rhythm (short heading, short body, image, proof line) -- not a new layout. Speaker's story, if the ad is first person, folded into an item's body or its proof line attributed to the ad speaker in her own words (see Attributed lines), never told in the author's own first person -- same rule as every other cartridge.
