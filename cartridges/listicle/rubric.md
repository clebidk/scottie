...
1. `style` is one of reasons/mistakes/questions/myths/tested and the headline follows the run's headline template (the style's formula unless the run names one from headlines.yaml), 8 to 14 words, no price. N equals the item count in every style; `tested`'s items report a claims check against verified specs, never a physical test.
...
3. Each item body is short -- one idea, a few sentences, the winner's density, no padding -- and closes with a `proof` line that either cites a verified claim_id or reads, to a reader, as an attributed customer statement.
...
9. No page-level word floor or band: the page reads at the winner's density (short, scannable items), never padded toward article length, and follows the winner's component map and punchiness rather than an invent-from-scratch longform.
10. One hero image plus one image per item, all distinct, lifestyle or installation over stock/illustration; none depict before/after or a clinical setting; image roles track the winner's slots (hero / item / lifestyle) when possible.
...
12. First-person attribution: if ad_brief.speaker_pov is first_person, the speaker's story is attributed to the ad speaker in her own words ("In the ad, she says ..."; see cartridge.md's Attributed lines) -- never told in the author's ({{ authors.author.name }}'s) own first person. Voice has no exclamation marks and none of: "game-changer", "unlock", "elevate", "journey".
