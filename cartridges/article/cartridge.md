...
- Warm-up window: keep pricing, the CTA, {{ tenant.name }}'s name, and every product / short_name out of the first {{ tenant.cartridges.article.warmup_window_words }} words (headline through turn_section inclusive) -- open, body_sections, alternatives_section, how_it_works_section, and turn_section stay brand-free. Name the company and product only in close. The reader earns the offer; the offer doesn't lead. This rule overrides any general "mention short_name on first section mention" voice guidance for this cartridge.
