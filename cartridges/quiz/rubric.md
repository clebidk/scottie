...
1. The headline is exactly "Which <category> Is Right for <audience>? Take the 60-Second Quiz"; the category names no company or model, the audience names people by situation or goal.
...
3. Each interstitial sits after the question the rubric names and teaches one verified fact on its topic in 1-3 sentences. Any number, price or trigger word carries a claim id.
...
8. With the script on: one question at a time, the progress bar moves, Back works, the result names one model with its verified price, capacity, image and CTA, and every "why this matches you" line is an answer the reader chose.
