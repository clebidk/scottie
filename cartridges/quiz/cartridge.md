...
## Structure (in order)
1. Header: the tenant disclosure label as an eyebrow (only when the tenant sets `disclosure_label`), H1, one-line dek, the start button ("Start the quiz", renderer text, a same-page anchor) with the renderer's note "<n> questions · about a minute", the one-line byline, and the hero image (`hero.asset_id`, one of the featured model's assets).
2. The quiz card, right under the header: "Question 2 of 6", a Back link and a thin progress bar, then one question at a time -- your rephrased prompt over the rubric's option tiles, which move on by themselves once one is chosen. After the questions the rubric names, one short interstitial line of yours, set as quiet text under a hairline. Without a script, every question and interstitial shows as one plain list.
3. The result: one card per active model, all hidden but the winner. Each card shows the model's title and descriptor (from its display name, e.g. "ACME One" over "2-Person Cabin", with the brand word in the tenant's own short-name form), its verified price, its product image, a "why it fits you" list (up to three of the reader's own answers that scored the model most) and the CTA to the model's page; under it, "See all models" (the tenant's `default_cta_url`) and "Retake the quiz". Without a script, the card for the model the ad featured shows, with a note that says so.
...
- Headline: exactly "Which <category> is right for <audience>?", in sentence case, with nothing after the question mark -- the page's own note says how long the quiz takes. `<category>` is the product category in lower case (e.g. "home infrared sauna"), never the company or a model name. `<audience>` names the people the ad speaks to by their situation or goal, 2-5 words, never a bare "people" or "buyers".
- Dek: one plain sentence of at most 20 words on what the quiz does for the reader -- no number unless it is cited, no "whether you're" opener, no list of three.
- Questions: exactly one entry per rubric question, in rubric order, with the rubric's `id`, a `prompt` you rephrase (one short question ending in "?", in the voice of the page), and `options`: the rubric's labels copied verbatim, same count, same order. (A tile shows at most six words: a longer rubric label carries its own short `display` text in the rubric, which the renderer shows -- you still copy the label.)
- Interstitials: exactly one entry per rubric interstitial, in order, `{"after": <question id>, "line": ..., "claim_ids": [...]}`. One or two short sentences, at most 25 words, that teach one verified fact on the rubric's topic. A line with a digit, a price, a measurement or a trigger word carries claim_ids; otherwise it has no digit at all. Never a health promise.
